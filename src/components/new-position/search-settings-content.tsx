...
export function SearchSettingsContent({ onDone, onExchangesClick, blockedExchangesCount, totalExchangesCount }: SearchSettingsContentProps) {
  const { translate } = useTranslation();
  const deleteIconRef = useRef<DeleteIconHandle>(null);
  const [minLiquidity, setMinLiquidity] = useState<string>(SearchSettingsConfig.default.minLiquidity);

  useEffect(() => {
    const config = LocalStorage.getSearchSettings();
    if (config.minLiquidity !== undefined) {
      setMinLiquidity(config.minLiquidity);
    }
  }, []);
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <h2 className="font-medium">{translate(AppTranslationsKeys.SEARCH_SETTINGS_MIN_LIQUIDITY_TITLE)}</h2>
              <InfoTooltip content={translate(AppTranslationsKeys.SEARCH_SETTINGS_MIN_LIQUIDITY_TOOLTIP)} />
            </div>
            {minLiquidity !== SearchSettingsConfig.default.minLiquidity && (
              <button
                onClick={() => handleMinLiquidityChange(SearchSettingsConfig.default.minLiquidity)}
          ...
          />
          <AnimatePresence>
            {Number(minLiquidity) < Number(SearchSettingsConfig.default.minLiquidity) && (
              <motion.p
              ...
              >
                {translate(AppTranslationsKeys.SEARCH_SETTINGS_LOW_LIQUIDITY_WARNING)}
              </motion.p>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="flex flex-col gap-2 pt-2 border-t border-divider">
          <PrimaryButton
            ...
            state={blockedExchangesCount === totalExchangesCount ? "destructive" : "default"}
            ...
            icon={
              <div className="relative">
                ...
                <AnimatePresence>{blockedExchangesCount > 0 && <Badge className="-top-0.5 -right-0.5" />}</AnimatePresence>
              </div>
            }
          >
            {translate(AppTranslationsKeys.EXCHANGES_FILTER_MODAL_TITLE)} ({totalExchangesCount - blockedExchangesCount}/{totalExchangesCount})
          </PrimaryButton>
        </div>
      </div>
    </AnimationProvider>
