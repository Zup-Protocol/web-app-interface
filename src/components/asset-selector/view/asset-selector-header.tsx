...
export function AssetSelectorHeader({ onBack, searchQuery, onSearchChange, onSearchClear, currentSelectedAsset, onDeselect, inputRef }: AssetSelectorHeaderProps) {
  const { translate } = useTranslation();

  return (
    <div className="bg-background/80 backdrop-blur-md w-full py-[11px] pointer-events-auto">
      <div className={cn("w-full flex items-center gap-4 mx-auto px-6 sm:px-0", "max-w-[500px]")}>
        <m.button
        ...
        >
          <ArrowLeft size={20} className="text-foreground group-hover:-translate-x-0.5 transition-transform" />
        </m.button>

        <div className="flex-1">
          <SearchInput
            ref={inputRef}
            placeholder={translate(AppTranslationsKeys.ASSET_SELECTOR_SEARCH_PLACEHOLDER)}
            value={searchQuery}
